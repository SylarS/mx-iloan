...
public class ForgetLoginPasswdActivity extends MVPBaseActivity implements ForgetView {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.forget_login_passwd);
        type = getIntent().getIntExtra("type", -1);
        if (type == Constant.LOGIN_PASSWD) {
            phoneNum = getIntent().getStringExtra("phoneNum");
            areaCode = getIntent().getStringExtra("areaCode");
            forgetPresenter.getVertifyCode(phoneNum, areaCode,Constant.VERTIFY_RESET);
        } else {
            if(AppStaus.userInfo!=null){
                areaCode=AppStaus.userInfo.getAreaCode();
            }
            forgetPresenter.getVertifyCode(AppStaus.userInfo.getPhoneNumber(), AppStaus.userInfo.getAreaCode(),Constant.VERTIFY_RESET);
        }
        initView();
        initListener();
    }
    ...
    protected void onStart() {
        super.onStart();
        setTitle(getResources().getString(R.string.forget_passwd));
    }
    ...
    private void initView() {
        ...
        mtoobar.setTitle(getResources().getString(R.string.forget_passwd));
        if (type == Constant.LOGIN_PASSWD) {
            mtvPhoneNum.setText(StringUtil.dealPhone(phoneNum));
        }else{
            if(AppStaus.userInfo!=null){
    ...
    private void initListener() {
        ...
        meditPw1.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                ...
                int len = editable.length();
                if (len == 1) {
                    InputUtil.hideSoftInput(ForgetLoginPasswdActivity.this, meditPw1);

                    InputUtil.showSoftInput(ForgetLoginPasswdActivity.this, meditPw2);
        ...
        meditPw2.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                ...
                int len = editable.length();
                if (len == 1) {
                    InputUtil.hideSoftInput(ForgetLoginPasswdActivity.this, meditPw2);
                    InputUtil.showSoftInput(ForgetLoginPasswdActivity.this, meditPw3);
                }
                  checkComplete();
            }
        });
        meditPw3.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                ...
                int len = editable.length();
                if (len == 1) {
                    InputUtil.hideSoftInput(ForgetLoginPasswdActivity.this, meditPw3);
                    InputUtil.showSoftInput(ForgetLoginPasswdActivity.this, meditPw4);
                }
                      checkComplete();
            }
        });
        meditPw4.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                ...
                int len = editable.length();
                if (len == 1) {
                    InputUtil.hideSoftInput(ForgetLoginPasswdActivity.this, meditPw4);
                ...
                if (!TextUtils.isEmpty(vertifyCode)) {
                    if (!TextUtils.isEmpty(passwd) && !TextUtils.isEmpty(rePasswd)) {
                        ...
                        if (passwd.matches(Constant.REGEX)) {
                            if (BoolUtil.fastDouble(ForgetLoginPasswdActivity.this)) {
                                return;
                            }
                          /*  StringBuilder sb = new StringBuilder();
                            ...
                            String phoneNum;
                            if(type==Constant.LOGIN_PASSWD){
                                phoneNum = ForgetLoginPasswdActivity.this.phoneNum;
                            }else{
                                phoneNum= AppStaus.userInfo.getPhoneNumber();
                            }

                            forgetPresenter.forget(Constant.LOGIN_PASSWD, passwd,vertifyCode, phoneNum,areaCode);
                        } else {
        ...
        if (!TextUtils.isEmpty(vertifyCode)) {
            ...
            if(!TextUtils.isEmpty(passwd)&&!TextUtils.isEmpty(rePasswd)){
               mbtnSet.setBackgroundResource(R.drawable.login_btn_shape);
                //ApkUtil.setBtnColor(ForgetLoginPasswdActivity.this,mbtnSet);
            }else {
    ...
    public void setPasswdSucess() {
        Intent intent=new Intent(ForgetLoginPasswdActivity.this,MainActivity.class);
